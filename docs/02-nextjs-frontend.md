...
3. **Data Fetching & API Integration**
   - Server Components: Use native `fetch` with `next: { revalidate: ... }` options.
   - Client Components: Utilize the pre-configured API client in `lib/api.ts` (if applicable) or call Route Handlers.
