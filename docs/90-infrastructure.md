...
| **Namespace**    | `gearpit-dev` / default      | `pr{number}`                   | `gearpit-core` / `gearpit-web`   |
| **Ingress Host** | `localhost` / `127.0.0.1`    | `*-pr{N}.192.168.40.100.nip.io` | `gearpit.192.168.40.100.nip.io`  |
| **DB Storage**   | Ephemeral (Empty on restart) | Ephemeral (Seeded via init)     | **Persistent (StatefulSet PVC)** |
| **Deployment**   | `skaffold` / `kubectl apply` | ArgoCD (ApplicationSet)         | ArgoCD (ApplicationSet)             |
