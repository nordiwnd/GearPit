    ...
    <div className="min-h-screen p-8 bg-zinc-50">
      <div className="max-w-6xl mx-auto space-y-8">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-zinc-800">Trip Plans</h1>
            <p className="text-zinc-500">Manage your expeditions and packing lists.</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 border-2 border-dashed rounded-lg">
            <h3 className="text-lg font-medium text-zinc-900">No trips planned yet</h3>
            <p className="text-zinc-500 mb-4">Start by creating a new packing plan.</p>
            <TripFormDialog onSuccess={fetchTrips} />
          </div>
        ) : (
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {trips.map((trip) => (
              <Link href={`/trips/${trip.id}`} key={trip.id} className="block group">
                <Card className="h-full transition-all hover:shadow-md hover:border-zinc-400">
                  <CardHeader>
                  ...
                  <CardFooter className="pt-2 border-t bg-zinc-50/50 rounded-b-xl group-hover:bg-blue-50/30 transition-colors">
                    <div className="w-full flex justify-between items-center text-sm font-medium text-zinc-500 group-hover:text-blue-600">
